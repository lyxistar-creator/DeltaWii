// DeltaWii - GameMaker: Studio runner for the Nintendo Wii.
//
...
// a second. Its whole texture pack is 40 MB, and a fight draws from most of it at once:
// the working set did not fit, so every frame evicted tiles it needed again immediately
// and re-read them off the card. A 2 MB tile fetched over USB several times a frame is
// not a frame rate problem, it is a disk problem wearing one as a disguise.
//
// Asking for 40 MB lets the entire pack be resident, which turns eviction off rather than
// merely making it rarer. MEM2 is 64 MB and the audio cache takes its own ~10 MB slice
// before this one; WiiTextures_init clamps the request to whatever the arena actually has
// left, so asking for more than exists is safe and simply gets less.
...
static void initVideo(void) {
    ...
    logInfo("DeltaWii: video up, %dx%d (xfb %d)\n",
...
// the old size until the file is closed, so a run that is killed rather than shut down
// cleanly leaves a log that reads as empty however much was written to it. Closing and
// reopening after each line updates the entry every time. It is far too slow to leave on
// once the game is drawing, so it is switched off when the main loop starts -- which is
// exactly when the log stops being about startup anyway.
...
// Lists one directory. Returns false when it cannot be opened at all.
//
// The names are printed exactly as libfat hands them back, which is the point: a card
// written by hand can hold a perfectly valid long-name record that the driver still
// declines to use, and then the directory reads as DELTAW~1 rather than deltawii. Seeing
// the name the driver actually has is the difference between knowing that and guessing.
static bool listDirectory(const char* path, int limit) {
    DIR* dir = opendir(path);
    if (dir == nullptr) {
        logError("  %-24s cannot be opened (errno %d)\n", path, errno);
        return false;
    }
    logError("  %-24s opened, contents:\n", path);
    int shown = 0;
    struct dirent* ent;
    while ((ent = readdir(dir)) != nullptr && shown < limit) {
...
// Walks a candidate game directory one component at a time, listing each level that
// opens and stopping at the first that does not.
//
// Listing only the roots -- which is all this used to do -- says whether the card is
// mounted and nothing else. When the card mounts and the game is still not found, the
// question is *which* level the driver loses the path at, and only walking down answers
// it. Every level is printed, not just the failing one, because the name a level reports
// for its child is itself the evidence.
static void walkCandidate(const char* full) {
    char path[128];
    size_t n = strlen(full);
    if (n >= sizeof(path)) return;

    // Start after the "device:/" prefix so the root itself is the first level listed.
    const char* colon = strchr(full, ':');
    size_t start = (colon != nullptr) ? (size_t)(colon - full) + 2 : 1;

    for (size_t i = start; i <= n; i++) {
        if (i != n && full[i] != '/') continue;
        memcpy(path, full, i);
        path[i] = '\0';
        if (i == 0) continue;
        if (!listDirectory(path, 16)) return;   // no point walking past a closed door
    }
}

// Reports what is mounted, then follows each place the game could be. A failure to find
// data.win should name the level that broke, not just say "not here".
static void dumpMountDiagnostics(void) {
    static const char* ROOTS[] = { "sd:/", "usb:/", "fat:/", "/" };

    logError("Mount points:\n");
    for (size_t i = 0; i < sizeof(ROOTS) / sizeof(ROOTS[0]); i++) {
        listDirectory(ROOTS[i], 12);
    }

    logError("Walking each candidate:\n");
    for (size_t i = 0; i < GAME_DIR_COUNT; i++) {
        // Only bother with candidates whose device mounted at all.
        char root[8];
        const char* colon = strchr(GAME_DIRS[i], ':');
        if (colon == nullptr) continue;
        size_t rl = (size_t)(colon - GAME_DIRS[i]) + 2;
        if (rl >= sizeof(root)) continue;
        memcpy(root, GAME_DIRS[i], rl);
        root[rl] = '\0';
        ...
        walkCandidate(GAME_DIRS[i]);
    }
}

// Why each candidate was rejected, so the failure path can say more than "not found".
// An ENOENT means the driver walked the path and found nothing; anything else means it
// got further and failed for a different reason, and the two want different fixes.
static int gOpenErrno[GAME_DIR_COUNT];
...
int main(int argc, char* argv[]) {
    ...
    if (!fatInitDefault()) {
        logError("DeltaWii: no FAT device found. Is the SD card inserted?\n");
        VIDEO_WaitVSync();
        sleep(5);
        return 1;
    }

    // Open a log at the card root *before* looking for the game, so that a failure to
    // find it is still written down somewhere readable.
    //
    // The main log lives next to data.win, which is no use when data.win is what could
    // not be found: the only record of that failure was the on-screen console, and
    // reading that means photographing a television. A boot log at a fixed path costs
    // one fopen and turns every startup failure into something recoverable off the card.
    {
        static const char* BOOT_LOGS[] = { "sd:/deltawii-boot.log", "usb:/deltawii-boot.log" };
        for (size_t i = 0; i < sizeof(BOOT_LOGS) / sizeof(BOOT_LOGS[0]); i++) {
            gLogFp = fopen(BOOT_LOGS[i], "w");
            if (gLogFp != nullptr) {
                gLogPath = safeStrdup(BOOT_LOGS[i]);
                logInfo("DeltaWii: boot log at %s\n", BOOT_LOGS[i]);
                break;
            }
        }
    }
    ...
    if (gameDir == nullptr) {
        logError("DeltaWii: data.win not found. Tried, with the errno each fopen gave:\n");
        for (size_t i = 0; i < GAME_DIR_COUNT; i++) {
            logError("  %-24sdata.win -> errno %d\n", GAME_DIRS[i], gOpenErrno[i]);
        }
        dumpMountDiagnostics();

        // Close the log before leaving. An emulator keeps the card in memory and only
        // writes it back when emulation ends cleanly, so a run that is killed at the
        // window leaves nothing behind however much was logged -- and on real hardware
        // an unflushed FILE* is just as lost.
        if (gLogFp != nullptr) { fclose(gLogFp); gLogFp = nullptr; }
        fatUnmount("sd:");
        fatUnmount("usb:");

        sleep(20);                                    // long enough to read the screen
        SYS_ResetSystem(SYS_RETURNTOMENU, 0, FALSE);  // and then leave, rather than hang
        return 1;
    }
    {
        // The game was found, so the boot log has done its job; hand over to the real one.
        if (gLogFp != nullptr) { fclose(gLogFp); gLogFp = nullptr; }

        free(gLogPath);
        gLogPath = (char*) safeMalloc(strlen(gameDir) + strlen("deltawii.log") + 1);
        strcpy(gLogPath, gameDir);
        strcat(gLogPath, "deltawii.log");
        gLogFp = fopen(gLogPath, "w");
    }

    logInfo("DeltaWii: game directory is %s\n", gameDir);
    ...
    logInfo("DeltaWii: loading %s ...\n", dataWinPath);
    DataWin* dataWin = DataWin_parse(dataWinPath, options);

    Gen8* gen8 = &dataWin->gen8;
    logInfo("DeltaWii: loaded \"%s\" (WAD version %u, GameMaker %u.%u.%u.%u)\n",
    ...
    if (audioSystem == nullptr) {
        logWarn("DeltaWii: audio failed to start, continuing without sound\n");
    ...
    // ===[ Texture pack ]===
    {
        ...
        if (!WiiTextures_init(packPath, TEXTURE_POOL_BYTES)) {
            logWarn("DeltaWii: no texture pack, the game will run but draw nothing.\n");
            logWarn("DeltaWii: generate it with tools/deltawii-preprocess.\n");
        }
        free(packPath);
    ...
    logInfo("DeltaWii: entering main loop\n");
    // From here the framebuffers belong to GX alone. Everything still reaches the SD log.
    gConsoleActive = false;

    // What the game itself asks to be stepped at. Undertale states 30 in every room;
    // Deltarune leaves the room field at 0 and puts 30.0 in GEN8, which the parser copies
    // into the rooms. Reading it rather than hardcoding 30 means a game that wants some
    // other rate still gets it.
    ...
    logInfo("DeltaWii: shutting down\n");
