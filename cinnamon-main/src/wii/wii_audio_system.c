...
// ===[ AUDO index ]===
//
// Deltarune splits its sounds across more than one file. Chapter 1 keeps 56 of them in
// data.win's own AUDO chunk and the other 86 in audiogroup1.dat, which is a bare FORM
// wrapper around a second AUDO chunk laid out identically. A sound's SOND entry names
// which group it belongs to, and its audioFile index is relative to that group -- so an
// index alone is ambiguous and every lookup has to carry the group with it.
typedef struct {
    uint32_t offset;   // absolute offset, within this group's file, of the WAV payload
    uint32_t size;
} AudoEntry;

// Group 0 is data.win itself; group N is audiogroupN.dat beside it.
#define WII_AUDIO_MAX_GROUPS 8

typedef struct {
    FILE*      fp;         // nullptr when the group is absent
    AudoEntry* entries;
    uint32_t   count;
} AudoGroupIndex;
...
typedef struct {
    ...
    int32_t  audoGroup;      // which group that index belongs to
...
typedef struct {
    ...
    AudoGroupIndex groups[WII_AUDIO_MAX_GROUPS];
...
// Walks a FORM file's chunks and records where every AUDO payload lives, without reading
// any of them. Mirrors what the texture cache does for TXTR. Works unchanged on data.win
// and on an audiogroup*.dat, because the audiogroup file is nothing but a FORM header
// wrapped around the same AUDO chunk.
//
// The file handle is kept open for the lifetime of the audio system: sounds are read out
// of it on demand, never held.
static bool indexAudoFile(WiiAudioSystem* wa, int32_t group, const char* path, bool required) {
    AudoGroupIndex* g = &wa->groups[group];

    g->fp = fopen(path, "rb");
    if (g->fp == nullptr) {
        if (required) logError("WiiAudio: cannot open %s\n", path);
        return false;
    }

    uint8_t hdr[8];
    if (fseek(g->fp, 0, SEEK_SET) != 0) return false;
    if (fread(hdr, 1, 8, g->fp) != 8) return false;
    if (memcmp(hdr, "FORM", 4) != 0) {
        logWarn("WiiAudio: %s has no FORM header\n", path);
    }
    ...
    while (p + 8 <= fileEnd) {
        uint8_t chunkHdr[8];
        fseek(g->fp, p, SEEK_SET);
        if (fread(chunkHdr, 1, 8, g->fp) != 8) break;
        uint32_t len = rdU32LE(chunkHdr + 4);
        if (memcmp(chunkHdr, "AUDO", 4) == 0) { audoAt = p + 8; break; }
        p += 8 + (long) len;
    }
    if (audoAt < 0) {
        if (required) logWarn("WiiAudio: no AUDO chunk in %s\n", path);
        fclose(g->fp);
        g->fp = nullptr;
        return false;
    }
    ...
    uint8_t countBuf[4];
    fseek(g->fp, audoAt, SEEK_SET);
    if (fread(countBuf, 1, 4, g->fp) != 4) return false;
    g->count = rdU32LE(countBuf);
    if (g->count == 0 || g->count > 65535u) {
        logWarn("WiiAudio: implausible AUDO count %u in %s\n", g->count, path);
        fclose(g->fp);
        g->fp = nullptr;
        g->count = 0;
        return false;
    ...
    uint8_t* ptrs = (uint8_t*) malloc(g->count * 4u);
    if (ptrs == nullptr) return false;
    if (fread(ptrs, 1, g->count * 4u, g->fp) != g->count * 4u) {
        free(ptrs);
        return false;
    }

    g->entries = (AudoEntry*) calloc(g->count, sizeof(AudoEntry));
    if (g->entries == nullptr) { free(ptrs); return false; }

    uint32_t largest = 0;
    for (uint32_t i = 0; i < g->count; i++) {
        uint32_t at = rdU32LE(ptrs + i * 4u);
        uint8_t lenBuf[4];
        fseek(g->fp, (long) at, SEEK_SET);
        if (fread(lenBuf, 1, 4, g->fp) != 4) continue;
        g->entries[i].size = rdU32LE(lenBuf);
        g->entries[i].offset = at + 4u;
        if (g->entries[i].size > largest) largest = g->entries[i].size;
    }
    free(ptrs);

    logInfo("WiiAudio: group %d indexed %u AUDO entries, largest %u KB (%s)\n",
            group, g->count, largest / 1024u, path);
    return true;
}

// Indexes data.win as group 0, then every audiogroupN.dat sitting beside it. A missing
// group is not an error: Undertale has none at all, and a chapter only ships the groups
// it uses. What is worth knowing is how many sounds ended up reachable, so that is logged.
static bool indexAudoGroups(WiiAudioSystem* wa, const char* dataWinPath) {
    bool any = indexAudoFile(wa, 0, dataWinPath, true);

    for (int32_t n = 1; n < WII_AUDIO_MAX_GROUPS; n++) {
        char path[512];
        snprintf(path, sizeof(path), "%saudiogroup%d.dat", wa->gameDir, (int) n);
        if (indexAudoFile(wa, n, path, false)) any = true;
    }
    return any;
}
...
static void initSlots(WiiAudioSystem* wa, uint32_t budget) {
    // One slot has to hold the biggest sound in *any* group, since a slot is not tied to
    // a group. Sizing this from data.win alone would leave audiogroup entries that no
    // slot can take, and they would fail to play with nothing to point at as the cause.
    uint32_t largest = 0;
    for (int32_t gi = 0; gi < WII_AUDIO_MAX_GROUPS; gi++) {
        AudoGroupIndex* g = &wa->groups[gi];
        for (uint32_t i = 0; i < g->count; i++) {
            if (g->entries[i].size > largest) largest = g->entries[i].size;
        }
    }
...
static int32_t acquireSlot(WiiAudioSystem* wa, int32_t audoGroup, int32_t audoIndex) {
    if (wa->slotCount == 0 || audoIndex < 0) return -1;
    if (audoGroup < 0 || audoGroup >= WII_AUDIO_MAX_GROUPS) return -1;

    AudoGroupIndex* g = &wa->groups[audoGroup];
    if (g->fp == nullptr || (uint32_t) audoIndex >= g->count) return -1;

    for (int32_t i = 0; i < wa->slotCount; i++) {
        if (wa->slots[i].audoIndex == audoIndex && wa->slots[i].audoGroup == audoGroup) {
    ...
    uint32_t size = g->entries[audoIndex].size;
    if (size > wa->slotBytes) return -1;

    if (fseek(g->fp, (long) g->entries[audoIndex].offset, SEEK_SET) != 0) return -1;
    if (fread(slot->base, 1, size, g->fp) != size) return -1;

    slot->audoIndex = audoIndex;
    slot->audoGroup = audoGroup;
    slot->pcm = nullptr;
    slot->pcmSize = 0;
    slot->refCount = 0;
    ...
    if (!parseWav(slot, size)) {
        logWarn("WiiAudio: AUDO entry %d of group %d is not 16-bit PCM WAV, skipping\n",
                audoIndex, audoGroup);
        slot->audoIndex = -1;
        return -1;
    }
...
static void wiiAudioDestroy(AudioSystem* audio) {
    ...
    ASND_End();
    for (int32_t i = 0; i < WII_AUDIO_MAX_GROUPS; i++) {
        if (wa->groups[i].fp != nullptr) fclose(wa->groups[i].fp);
        free(wa->groups[i].entries);
    }
    free(wa->slots);
    free(wa->gameDir);
    free(wa);
...
static int32_t wiiAudioPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
    ...
    if (soundIndex >= WII_AUDIO_STREAM_INDEX_BASE) {
        ...
        if (embedded && sound->audioFile >= 0) {
            ...
            // audioFile is an index within the sound's own group, not a global one.
            audoGroup = (sound->audioGroup > 0) ? sound->audioGroup : 0;
        } else {
    ...
    if (streamFile != nullptr) {
        ...
        int32_t slot = acquireSlot(wa, audoGroup, audoIndex);
        if (slot < 0) {
            releaseInstance(wa, inst);
            return -1;
        }
...
AudioSystem* WiiAudioSystem_create(const char* gameDir, const char* dataWinPath) {
    ...
    if (indexAudoGroups(wa, dataWinPath)) {
