...
// Deltarune keeps its music in mus/ and its text in lang/, so the copy has to recurse.
// Undertale had a flat game directory and this walked one level; a silently skipped
// subdirectory here would mean a card that boots and then has no music.
let written = 0;
function copyTree(srcPath, dirCluster, prefix) {
    for (const name of fs.readdirSync(srcPath).sort()) {
        const full = path.join(srcPath, name);
        const stat = fs.statSync(full);
        if (stat.isDirectory()) {
            const child = ensureDir(fat, dirCluster, name);
            console.log(`  ${(prefix + name + "/").padEnd(28)} ${"".padStart(8)}      -> cluster ${child}`);
            copyTree(full, child, prefix + name + "/");
            continue;
        }
        if (!stat.isFile()) continue;
        const data = fs.readFileSync(full);
        const action = writeFile(fat, dirCluster, name, data);
        written += data.length;
        console.log(`  ${(prefix + name).padEnd(28)} ${(data.length / 1024 / 1024).toFixed(2).padStart(8)} MB  ${action}`);
    }
}
copyTree(srcDir, dir, "");
